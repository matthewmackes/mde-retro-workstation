...
**Theme routing (E0 dependency):** all colors flow through `palette::color()`; `Theme::Windows10` + `win10(rgb)` (added by E0) gives the Win10 accent/light-dark look for free. E3 adds **no raw hex** (§2.1); any new accent/urgency color it needs is added as a role constant in `palette.rs` and remapped in `win10()`/`carbon()`. Critical urgency (`hint_urgency >= 2`) pins the toast open (suppresses auto-dismiss) AND tints it danger-red — a red summary + a 2px red border — via the `palette::URGENT` role, which `carbon()`/`win10()` remap to a Carbon danger red (`#da1e28` light / `#fa4d56` dark). (E3-urgent-tint; `URGENT` was carried by the original scaffold, removed as unreachable in the second audit sweep, then re-added and wired here.) Every `.size()` uses `metrics::UI_PX`/`INFO_TITLE_PX` (§2.3).
