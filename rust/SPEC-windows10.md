...
**Theme routing (E0 dependency):** all colors flow through `palette::color()`; `Theme::Windows10` + `win10(rgb)` (added by E0) gives the Win10 accent/light-dark look for free. E3 adds **no raw hex** (§2.1); any new accent/urgency color it needs is added as a role constant in `palette.rs` and remapped in `win10()`/`carbon()`. Critical urgency (`hint_urgency >= 2`) currently pins the toast open (suppresses auto-dismiss) so it stays until acted on; a dedicated danger-red "urgent" tint is a tracked follow-up (see the worklist), not yet wired — the `palette::URGENT` role that earlier scaffolding carried for it was removed as unreachable in the second audit sweep. Every `.size()` uses `metrics::UI_PX`/`INFO_TITLE_PX` (§2.3).
